/* VendorBridge Manager Module — Shared JS */
...
function requireManager(){
  ...
  if(!token||role!=='manager'){ window.location.href='/pages/login.html'; return false; }
  return true;
}
...
function showToast(msg,type='info',dur=3500){
  ...
  t.innerHTML=`${TI[type]||''}<span style="flex:1">${msg}</span><button class="toast-close" onclick="this.parentElement.remove()"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`;
  _tc.appendChild(t);
...
function formatNumber(n){ if(n==null)return'0'; if(n>=1e7)return(n/1e7).toFixed(1)+'Cr'; if(n>=1e5)return(n/1e5).toFixed(1)+'L'; if(n>=1e3)return(n/1e3).toFixed(1)+'K'; return String(n); }
...
function statusBadge(s){
  if(!s)return'';
  const m={pending:'pending',approved:'approved',rejected:'rejected',draft:'low',active:'active',
    quotation_received:'info',pending_approval:'pending',po_generated:'success',
    invoice_generated:'success',closed:'low',cancelled:'error',
    low:'low',normal:'normal',high:'high',urgent:'urgent',
    overdue:'overdue',due_soon:'due_soon',on_track:'on_track'};
  return`<span class="badge badge-${m[s]||s}">${s.replace(/_/g,' ')}</span>`;
}

function priorityBadge(p){
  const icons={low:'↓',normal:'–',high:'↑',urgent:'🔴'};
  return`<span class="badge badge-${p}">${icons[p]||''} ${p}</span>`;
}
...
function renderPagination(containerId,pagination,onPageChange){
  ...
  if(start>1) html+=`<button class="page-btn" onclick="(${onPageChange.toString()})(1)">1</button>${start>2?'<span style="padding:0 4px;color:var(--muted)">…</span>':''}`;
  for(let i=start;i<=end;i++) html+=`<button class="page-btn ${i===page?'active':''}" onclick="(${onPageChange.toString()})(${i})">${i}</button>`;
  if(end<pages) html+=`${end<pages-1?'<span style="padding:0 4px;color:var(--muted)">…</span>':''}<button class="page-btn" onclick="(${onPageChange.toString()})(${pages})">${pages}</button>`;
...
async function loadUserInfo(){
  try{
    ...
    if(d.success&&d.user){
      ...
      const el=document.getElementById('userName');if(el)el.textContent=d.user.full_name;
      const av=document.getElementById('userAvatar');if(av)av.textContent=d.user.full_name.charAt(0).toUpperCase();
    }
  }catch(e){}
}
...
async function updateNotifBadge(){
  try{
    ...
    if(b){b.textContent=d.unread_count>99?'99+':d.unread_count;b.style.display=d.unread_count>0?'inline-block':'none';}
    const dot=document.getElementById('notifDot');
    if(dot)dot.style.display=d.unread_count>0?'block':'none';
    // Update pending approvals badge
    const pb=document.getElementById('pendingBadge');
    if(pb){
      const ps=await apiGet('/dashboard/stats');
      if(ps.success&&ps.data.pending_approvals>0){
        pb.textContent=ps.data.pending_approvals;pb.style.display='inline-block';
...
function drawBarChart(canvasId,labels,datasets,opts={}){
  ...
  const pad={top:20,right:16,bottom:36,left:52},cW=W-pad.left-pad.right,cH=H-pad.top-pad.bottom;
  const maxVal=Math.max(...datasets.flatMap(d=>d.data),1);
  const barW=Math.max(4,(cW/labels.length)*0.4/Math.max(datasets.length,1));
  const gap=cW/labels.length;
  ctx.strokeStyle='#e2e8f0';ctx.lineWidth=1;
  for(let i=0;i<=4;i++){const y=pad.top+(i/4)*cH;ctx.beginPath();ctx.moveTo(pad.left,y);ctx.lineTo(pad.left+cW,y);ctx.stroke();ctx.fillStyle='#94a3b8';ctx.font='10px Inter';ctx.textAlign='right';ctx.fillText(Math.round(maxVal*(1-i/4)),pad.left-6,y+4);}
  ...
  labels.forEach((lbl,i)=>ctx.fillText(lbl,pad.left+i*gap+gap/2,H-10));
}
...
function initManagerPage(activeNav){
