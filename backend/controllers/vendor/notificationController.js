...
exports.markAsRead = async (req, res) => {
  try {
    ...
    return res.json({ success: true, unread_count: await getUnreadCount(req.user.id) });
  } catch (e) { return res.status(500).json({ success: false, message: 'Failed.' }); }
};

exports.getUnreadCount = async (req, res) => {
  try { return res.json({ success: true, unread_count: await getUnreadCount(req.user.id) }); }
  catch (e) { return res.status(500).json({ success: false, message: 'Failed.' }); }
};
